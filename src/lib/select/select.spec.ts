...
describe('MatSelect', () => {
      ...
      describe('for select', () => {
        ...
        it('should not set an aria-label if aria-labelledby is specified', fakeAsync(() => {
             ...
             expect(select.getAttribute('aria-label'))
                 .toBeFalsy('Expected no aria-label to be set.');
        ...
        it('should not shift focus when the selected options are updated programmatically ' +
           ...
           fakeAsync(() => {
             ...
             expect(document.activeElement)
                 .toBe(options[3], 'Expected fourth option to be focused.');
      ...
      describe('for option groups', () => {
        ...
        it('should set the `aria-labelledby` attribute', fakeAsync(() => {
             ...
             expect(group.getAttribute('aria-labelledby'))
                 .toBe(
                     label.getAttribute('id'), 'Expected `aria-labelledby` to match the label id.');
           }));
    ...
    describe('selection logic', () => {
      ...
      it('should remove selection if option has been removed', fakeAsync(() => {
           ...
           expect(select.selected)
               .toBe(select.options.first, 'Expected first option to be selected.');
    ...
    describe('forms integration', () => {
      ...
      it('should take an initial view value with reactive forms', fakeAsync(() => {
           ...
           expect(value.nativeElement.textContent)
               .toContain(
                   'Pizza', `Expected trigger to be populated by the control's initial value.`);
      ...
      it('should clear the selection when a nonexistent option value is selected', fakeAsync(() => {
           ...
           expect(trigger.textContent)
               .not.toContain(
                   'Pizza', `Expected trigger is cleared when option value is not found.`);
           ...
           expect(options[1].classList)
               .not.toContain(
                   'mat-selected', `Expected option w/ the old value not to be selected.`);
         }));
      ...
      it('should set the control to touched when the select is blurred', fakeAsync(() => {
           ...
           expect(fixture.componentInstance.control.touched)
               .toEqual(
                   true, `Expected the control to be touched as soon as focus left the select.`);
         }));
      ...
      it('should set an asterisk after the label if control is required', fakeAsync(() => {
           let requiredMarker =
               fixture.debugElement.query(By.css('.mat-form-field-required-marker'));
  ...
  describe('with floatLabel', () => {
    ...
    it('should default to global floating label type', fakeAsync(() => {
         ...
         TestBed.configureTestingModule({
           imports: [
             MatFormFieldModule, MatSelectModule, ReactiveFormsModule, FormsModule,
             NoopAnimationsModule
           ],
  ...
  describe('with custom trigger', () => {
    ...
    it('should allow the user to customize the label', fakeAsync(() => {
         ...
         expect(label.textContent)
             .toContain('azziP', 'Expected the displayed text to be "Pizza" in reverse.');
       }));
  });
  ...
  describe('positioning', () => {
    ...
    function checkTriggerAlignedWithOption(
        index: number, selectInstance = fixture.componentInstance.select): void {
      ...
      expect(Math.abs(expectedOrigin - origin) < 2)
          .toBe(true, `Expected panel animation to originate in the center of option ${index}.`);
    }
    ...
    describe('limited space to open vertically', () => {
      ...
      it('should adjust position of centered option if there is little space above',
         fakeAsync(() => {
           ...
           const topMostPositionForPerfectCentering = idealSpaceAboveSelectedItem +
               selectMenuViewportPadding + (selectItemHeight - triggerHeight) / 2;
      ...
      it('should adjust position of centered option if there is little space below',
         fakeAsync(() => {
           ...
           const bottomMostPositionForPerfectCentering = idealSpaceAboveSelectedItem +
               selectMenuViewportPadding + (selectItemHeight - triggerHeight) / 2;
    ...
    describe('limited space to open horizontally', () => {
      ...
      it('should stay within the viewport when overflowing on the left in ltr', fakeAsync(() => {
           ...
           const panelLeft =
               document.querySelector('.mat-select-panel')!.getBoundingClientRect().left;

           expect(panelLeft).toBeGreaterThan(
               0, `Expected select panel to be inside the viewport in ltr.`);
         }));
      ...
      it('should stay within the viewport when overflowing on the left in rtl', fakeAsync(() => {
           ...
           expect(panelLeft).toBeGreaterThan(
               0, `Expected select panel to be inside the viewport in rtl.`);
         }));
      ...
      it('should stay within the viewport when overflowing on the right in ltr', fakeAsync(() => {
           ...
           const panelRight =
               document.querySelector('.mat-select-panel')!.getBoundingClientRect().right;

           expect(viewportRect - panelRight)
               .toBeGreaterThan(0, `Expected select panel to be inside the viewport in ltr.`);
         }));
      ...
      it('should stay within the viewport when overflowing on the right in rtl', fakeAsync(() => {
           ...
           expect(viewportRect - panelRight)
               .toBeGreaterThan(0, `Expected select panel to be inside the viewport in rtl.`);
         }));
      ...
      it('should keep the position within the viewport on repeat openings', fakeAsync(() => {
           ...
           expect(panelLeft).toBeGreaterThan(
               0, `Expected select panel continue being inside the viewport.`);
         }));
    });
    ...
    describe('when scrolled', () => {
      ...
      it('should align the first option properly when scrolled', fakeAsync(() => {
           ...
           // not successfully constrain its size. As such, skip assertions in environments where
           // the window size has changed since the start of the test.
    ...
    describe('x-axis positioning', () => {
      ...
      it('should align the trigger and the selected option on the x-axis in rtl', fakeAsync(() => {
           ...
           const firstOptionRight =
               document.querySelector('.cdk-overlay-pane mat-option')!.getBoundingClientRect()
                   .right;
    ...
    describe('x-axis positioning with groups', () => {
      ...
      it('should adjust for the group padding in ltr', fakeAsync(() => {
           ...
           groupFixture.whenStable().then(() => {
             ...
             const triggerLeft = trigger.getBoundingClientRect().left;
             const selectedOptionLeft =
                 group.querySelector('mat-option.mat-selected')!.getBoundingClientRect().left;

             // 32px is the 16px default padding plus 16px of padding when an option is in a group.
             expect(Math.floor(selectedOptionLeft))
                 .toEqual(
                     Math.floor(triggerLeft - 32),
      ...
      it('should adjust for the group padding in rtl', fakeAsync(() => {
           ...
           const group = document.querySelector('.cdk-overlay-pane mat-optgroup')!;
           const triggerRight = trigger.getBoundingClientRect().right;
           const selectedOptionRight =
               group.querySelector('mat-option.mat-selected')!.getBoundingClientRect().right;

           // 32px is the 16px default padding plus 16px of padding when an option is in a group.
           expect(Math.floor(selectedOptionRight))
               .toEqual(
                   Math.floor(triggerRight + 32),
      ...
      it('should align the first option to the trigger, if nothing is selected', fakeAsync(() => {
           ...
           // There appears to be a small rounding error on IE, so we verify that the value is
           // close, not exact.
           if (platform.TRIDENT) {
             let difference =
  ...
  describe('with multiple selection', () => {
    ...
    it('should pass the `multiple` value to all of the option instances', fakeAsync(() => {
         ...
         expect(testInstance.options.toArray().every(option => !!option.multiple))
             .toBe(true, 'Expected `multiple` to have been added to initial set of options.');

         testInstance.foods.push({value: 'cake-8', viewValue: 'Cake'});
         fixture.detectChanges();

         expect(testInstance.options.toArray().every(option => !!option.multiple))
             .toBe(true, 'Expected `multiple` to have been set on dynamically-added option.');
       }));
    ...
    it('should select all options when pressing ctrl + a', () => {
      ...
      expect(testInstance.control.value).toEqual([
        'steak-0', 'pizza-1', 'tacos-2', 'sandwich-3', 'chips-4', 'eggs-5', 'pasta-6', 'sushi-7'
      ]);
    });
...
@Component({
  ...
  providers: [{provide: NG_VALUE_ACCESSOR, useExisting: CustomSelectAccessor, multi: true}]
})
...
@Component({selector: 'throws-error-on-init', template: ''})
...
class SelectWithGroupsAndNgContainer {
  control = new FormControl();
  pokemonTypes = [{name: 'Grass', pokemon: [{value: 'bulbasaur-0', viewValue: 'Bulbasaur'}]}];
}
...
class SelectInsideFormGroup {
  ...
  formGroup = new FormGroup({food: this.formControl});
}
